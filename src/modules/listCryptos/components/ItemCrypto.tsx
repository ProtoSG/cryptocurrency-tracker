...
export default function ItemCrypto({ crypto }: ItemCryptoProps) {
  ...
  return (
    <article
      ...
      className={`flex items-center justify-between transition-colors gap-4 px-8 py-4 hover:cursor-pointer hover:bg-zinc-700 
    ${cryptoSelected?.id === crypto.id ? 'bg-zinc-700 sticky top-0' : ''}`}>
      <div className="flex justify-between  md:w-full md:justify-center lg:justify-between">
